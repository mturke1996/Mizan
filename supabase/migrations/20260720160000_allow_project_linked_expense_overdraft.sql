-- Project-linked wallet expenses may overdraw the wallet.
-- Non-project expenses still require sufficient funds.
-- Covers both post_transaction overloads (with/without business_client_id).
...
begin
  ...
  -- Project expenses may overdraw; personal/workspace expenses may not.
  if p_kind = 'expense'
...
-- Legacy overload without business_client_id
create or replace function public.post_transaction(
  p_workspace_id uuid,
  p_client_id uuid,
  p_wallet_id uuid,
...
grant execute on function public.post_transaction(
  uuid, uuid, uuid, public.transaction_kind, bigint, timestamptz, text, uuid, uuid
) to authenticated;
